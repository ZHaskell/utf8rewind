    ...
    def offsetToString(self, offset, pageOffsets):
        ...
        for p in range(0, len(pageOffsets)):
            if pageOffsets[p] > offset:
                break
            page_offset = p
        return hex((int(page_offset) << 24) | int(offset - pageOffsets[page_offset]))
...
class Normalization(libs.unicode.UnicodeVisitor):
    ...
    def writeSource(self, filepath):
        ...
        while 1:
            ...
            blob_page = blob_page[int(page_read * 4):]
            blob_size -= page_read
        ...
        for p in range(0, pages):
            blob_page = self.blob[int(page_starts[p] * 4):int(page_ends[p] * 4)]
            ...
            while (1):
                ...
                character_line = blob_sliced[:int(character_count * 4)]
